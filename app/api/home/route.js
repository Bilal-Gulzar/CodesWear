...
try{
  ...
  const homePageProducts = products.filter(
    (v) =>
      ...
      v.slug === "your-design-here-hoodie-navyblue-s"
  );

  const p1 = homePageProducts.slice(0,3)
  const p2 = homePageProducts.slice(3,5)
  const p3 = homePageProducts
 return Response.json({ success:true, product1:p1, product2:p2 ,product3:p3}) 
}
catch(error){
