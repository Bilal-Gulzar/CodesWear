...
export default async function Hoodies({ searchParams }) {
  ...
  const Allproducts = await getHoodies(page);
  const hoodies = Allproducts.tshirts;
                    ...
                    <div className="dark:bg-[#374151] relative rounded-b-lg -left-4 h-[43%] w-[114.6%]">
                      <div className="relative dark:top-7  top-5 left-5">
                        <h3 className="text-gray-500 dark:text-gray-400 text-xs tracking-widest font-sans font-medium title-font mb-1">
                          {hoodies[v].category.toString().toUpperCase()}
                        </h3>
                        <h2 className="text-gray-900 dark:text-gray-100 title-font font-sans text-lg font-medium">
                          {hoodies[v].tittle.length > 21 ? (
                            <span>{hoodies[v].tittle.substr(0, 21)}...</span>
                          ) : (
                            hoodies[v].tittle
                          )}
                        </h2>
                      ...
                      <div className=" relative top-8 dark:top-12 left-5">
                        {hoodies[v].size.includes("S") && (
                          <span className="text-md border-2 py-1 font-sans text-black px-2 mx-[2px] dark:text-gray-100">
                            S
                          </span>
                        )}
                        {hoodies[v].size.includes("M") && (
                          <span className="text-md border-2 py-1 font-sans text-black px-2 mx-[2px] dark:text-gray-100">
                            M
                          </span>
                        )}
                        {hoodies[v].size.includes("L") && (
                          <span className="text-md border-2 py-1 font-sans text-black px-2 mx-[2px] dark:text-gray-100">
                            L
                          </span>
                        )}
                        {hoodies[v].size.includes("XL") && (
                          <span className="text-md border-2 py-1 font-sans text-black px-2 mx-[2px] dark:text-gray-100">
                            XL
                          </span>
                        )}
                        {hoodies[v].size.includes("XLL") && (
                          <span className="text-md border-2 py-1 font-sans text-black px-2 mx-[2px] dark:text-gray-100">
                            XXL
                          </span>
                        )}
